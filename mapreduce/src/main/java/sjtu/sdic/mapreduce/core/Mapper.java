...
public class Mapper {
    ...
    public static void doMap(String jobName, int mapTask, String inFile, int nReduce, MapFunc mapF) {
        mapF.map(inFile, readFile(inFile))
                .stream()
                .collect(Collectors.groupingBy(kv -> hashCode(kv.key) % nReduce))
                .forEach((reduceTask, value) -> writeFile(Utils.reduceName(jobName, mapTask, reduceTask), JSON.toJSONString(value)));
    }
    ...
    public static void writeFile(String file, String contents) {
        try {
            ...
            BufferedWriter writer = Files.newBufferedWriter(Paths.get(temp));
            writer.write(contents);
            writer.close();
            Files.move(Paths.get(temp), Paths.get(file), StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
